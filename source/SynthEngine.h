...
public:
    ...
    StereoSample render(const RuntimeParams& p)
    {
        ...
        const float oscALevel = p.oscALevel <= 0.001f ? 0.0f : 0.82f * std::pow(p.oscALevel, 1.55f);
        const float oscBLevel = p.oscBLevel <= 0.001f ? 0.0f : 0.78f * std::pow(p.oscBLevel, 1.60f);
        const float subLevel = p.subLevel <= 0.001f ? 0.0f : 0.42f * std::pow(p.subLevel, 1.70f);
        const float noiseLevel = p.noiseLevel <= 0.001f ? 0.0f : 0.10f * std::pow(p.noiseLevel, 2.25f);
        ...
        subToneState += 0.06f * (subSample - subToneState);
        ...
        const float onsetLen = juce::jmax(32.0f, 0.0060f * (float) sampleRate);
